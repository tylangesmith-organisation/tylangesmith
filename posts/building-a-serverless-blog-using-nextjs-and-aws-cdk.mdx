...
> S3 Bucket names are required to be globally unique, along with a few other [rules](https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucketnamingrules.html)
...
Now our users can access our website via something a bit more well-known `https://tylangesmith.com`.
...
However, a problem arises when the user requests the `https://tylangesmith.com/blog/some-blog` URL without the Next.js library being loaded. E.g. when refreshing the page.
...
We can trigger this Lambda@Edge function on the required paths, which will map the incoming request to the correct related file within our S3 bucket.

Phew, a bit to take in there, but we've done well. At this point, we have a solution that will scale with our demand, is highly performant and is cost optimised.
