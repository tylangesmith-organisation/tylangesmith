...
Now, we need to create an S3 Static Website Bucket. We can create a simple helper function in our stack to abstract this. We'll also need to define a simple function that generates the URL for our branch. We can then use this URL as our bucket name.

<Gist id="d65b5252912c43964197274f7cec1ed2" />

Our URL branch generation code is pretty straightforward, similar to our stack name generation.

<Gist id="010a0f4dea981d149865a225cc43ff8c" />

And finally, our `createStaticWebsiteBucket` code.

<Gist id="83de43f4fad6a958195753221a44b3b0" />

By default, this bucket denies all public access. This bucket will also have static website hosting enabled because we've specified the `websiteIndexDocument` and `websiteErrorDocument` documents. 

One last thing to note here as well is we've set the `autoDeleteObjects` and `removalPolicy` to be pretty destructive when we delete the CloudFormation stack. This makes the cleanup of branches a lot easier; however, it might be worth turning this off for the production stack to ensure we don't easily delete our production bucket.

## Creating an SSL Certificate

Now, we need to create our SSL certificate to use with our CloudFront Distribution. To do this, we first need to grab a reference to our domain's hosted zone in Route53. Let's define these abstractions in our stack.

<Gist id="e4778832977e144d1658ced51e69591f" />

Our Route53 hosted zone lookup is simple.

<Gist id="1661ac3b5aa733aaa5aa89d90f1eb2ca" />

And our certificate creation is pretty simple too.

<Gist id="a71be2bc42ca0c97076166bd30970e0d" />

To create the certificate, AWS needs to ensure that we actually manage the Route53 hosted zone. For this, we can use the `DnsValidatedCertificate` construct, which will automatically validate our hosted zone.

## Creating a CloudFront Function

Next, let's go ahead and create our CloudFront Function. This allows us to define a lightweight function that we can run when a user requests content from our CloudFront Distribution.

<Gist id="5b0e2a1e20448865cf05dc13b4542213" />

Defining the `createFunction` function.

<Gist id="2184f20bf0e94b8435957c0fbb933a55" />

This `Function` construct grabs our lightweight code to run from a local file we've defined `./src/functions/mapperFunction.js`.

<Gist id="2d4a9bcac4638ae95c7ea7c4e9bb1939" />

This pretty straightforward mapping maps URI's that don't map to a file to their respective `.html` file.
