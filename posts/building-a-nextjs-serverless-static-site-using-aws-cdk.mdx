...
In this blog, we'll explore how to host a Next.js static site on AWS. We'll use AWS CloudFront, CloudFront Functions, and S3 to implement a fully serverless solution. We'll then take this serverless solution and leverage AWS CDK to define our infrastructure as code.
...
We'll explore a simple example solution I've implemented [nextjs-serverless-static-site.tylangesmith.com](https://nextjs-serverless-static-site.tylangesmith.com).

This is a simple static website built using [Next.js](https://nextjs.org). Next.js comes with a set of pre-rendering [static generation](https://nextjs.org/docs/basic-features/pages#static-generation-recommended) features right out of the box that suits this exact use case. We won't deep-dive into this Next.js implementation, but for the context of this blog, the end result is a small static bundle that we want to deploy.

And of course, all of the code for this can be found on my [GitHub](https://github.com/tylangesmith-organisation/nextjs-serverless-static-site)!
...
Awesome! So we want to develop a hosting solution for our Next.js static site. We want a solution that'll be performant,  scalable, reliable, secure, and cost-optimised. Naturally, we also want to try and keep things relatively simple.

Thankfully, since our website is static, we can meet these requirements without too much hassle.

It's also worth mentioning that hosting solutions like [Vercel](https://vercel.com/) make deploying a Next.js project ridiculously easy (plus a bunch of other frameworks), not to mention it provides a bunch of optimisations right out of the box. From a developer experience perspective, this is amazing and well worth consideration.
...
<Image
  source="buildingAServerlessBlogUsingNextjsAndAwsCdk/solutionArchitecture.png"
...
<Image
  source="buildingAServerlessBlogUsingNextjsAndAwsCdk/lambda@edgeFunction.png"
