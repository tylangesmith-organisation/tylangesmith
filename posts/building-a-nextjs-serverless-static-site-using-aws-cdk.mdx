...
- `branchName`: Our current Git branch name is used to generate resources with unique names for the branch, enabling testing the branch in isolation. e.g. I can test the branch `my-feature-1` separately from `my-other-feature`.

- `domainName`: The base domain name of the URL for our project. e.g. `tylangesmith.com`.

- `subDomainName`: The subdomain name of the URL for our project. e.g. `nextjs-serverless-static-site`.

As you can see, we generate a `stackName` from our `GITHUB_REPOSITORY` e.g. `tylangesmith-organisation/nextjs-serverless-static-site` and our `BRANCH_NAME`. For this we've created a `getStackName` function - let's define it.
