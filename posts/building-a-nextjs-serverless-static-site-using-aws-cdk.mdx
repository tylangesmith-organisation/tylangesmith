...
This defines our `StaticWebsiteStack` class which extends the base `Stack` CDK class and will contain all of our static website resources. 
...
- `branchName`: Our current Git branch name e.g. `my-feature-1`.

- `domainName`: The domain name for our project. e.g. `tylangesmith.com`.

- `subDomainName`: The subdomain for our project. e.g. `nextjs-serverless-static-site`.

- `env`: A few extra variables to correctly enable route53 lookups to work.

As you can probably see, we generate our `stackName` from our `GITHUB_REPOSITORY` e.g. `tylangesmith-organisation/nextjs-serverless-static-site` and our `BRANCH_NAME`. Why? Well, let's explore this.
...
Now, we need to create an S3 Bucket. For this, we need to give our bucket a unique name. Since we need to generate our URL with branching support anyway, we can use this as our bucket name.

<Gist id="d65b5252912c43964197274f7cec1ed2" />

To generate our branched URL, we have the following code. This is pretty similar to our stack name generation code.

<Gist id="010a0f4dea981d149865a225cc43ff8c" />

The main thing to note here is if we are not on our default `master` branch, the branch name will be suffixed onto our subdomain name. However, if we are on the default `master` branch, we'll return the subdomain name without any suffix.

Now let's define our actual bucket creation code. 

<Gist id="83de43f4fad6a958195753221a44b3b0" />

Here we use the `Bucket` construct. This bucket denies all public access by default. Since we will use a CloudFront distribution, we don't need to enable static website hosting. However, it is worth mentioning that we can enable static website hosting and put it behind a CloudFront distribution, but this has a few nuances we need to consider when defining the AWS CDK code.
...
Next, we need to create an SSL certificate to use with our CloudFront Distribution. To do this, we need to specify which Route53 hosted zone we want to issue the certificate for.

<Gist id="e4778832977e144d1658ced51e69591f" />

Here we define 2 functions, a `getHostedZone` function that will grab a reference to the Route53 hosted zone for our domain and a `createCertificate` function that will create the certificate.

The lookup for our Route53 hosted zone is pretty simple.

> Note for this to work, we need the extra `env` variables (`account` and `region`) to be passed to our stack in our `index.ts` file.

<Gist id="1661ac3b5aa733aaa5aa89d90f1eb2ca" />

Our certificate creation is pretty simple too.

<Gist id="a71be2bc42ca0c97076166bd30970e0d" />

Here we use the `DnsValidatedCertificate` construct. This construct issues a certificate for the domain we specify. However, to create this certificate, AWS needs to ensure we actually manage the Route53 hosted zone. Thankfully this construct will automatically validate this for us.

## Creating a CloudFront Function

Next up, we need to create our CloudFront Function. This allows us to correctly map any non-html translated Next.js URLs to their respective `.html` files. 

For this, we can run lightweight code on specific user requests.

<Gist id="5b0e2a1e20448865cf05dc13b4542213" />

The main thing to note here is that we specify the `filePath` of our code to run.

Now lets define `createFunction`.

<Gist id="2184f20bf0e94b8435957c0fbb933a55" />

Here we use the `Function` construct with the `FunctionCode` utility. This will grab the code we've defined at `./src/functions/mapperFunction.js`. 

<Gist id="2d4a9bcac4638ae95c7ea7c4e9bb1939" />

This mapping code modifies the incoming request URI.

## Creating a CloudFront Distribution

Ok, we're finally at the point where we can start using all of these components to create our CloudFront Distribution.

As you'd expect, we call a simple function that will create our CloudFront Distribution from our stack class `createDistribution`.
...
This construct mainly pulls together all of our previously created resources. First, we ensure we can access our distribution via our URL. To do this, we specify an alternate domain name with `domainName` and provide the issued `certificate` for the URL.

Next, we need to define how our distribution behaves; we can do this with the `defaultBehavior` property. In our case, we want to serve static content from our bucket so we can define an `origin` that points to our S3 bucket. We also need to associate this behaviour with our CloudFront Function, which we only want to run what a user requests to view content.
