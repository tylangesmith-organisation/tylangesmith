...
## Creating an S3 Static Website Bucket

Now, we need to create an S3 Static Website Bucket. For this, we need to give our bucket a unique name. Since we need to generate our URL with branching support anyway, we can use this as our bucket name.
...
Here we use the `Bucket` construct. This bucket denies all public access by default. Static website hosting will also be enabled because we've specified the `websiteIndexDocument` and `websiteErrorDocument` documents. 
...
This mapping code modifies the incoming request URI. This will only run on a specified set of paths we'll define later on.
